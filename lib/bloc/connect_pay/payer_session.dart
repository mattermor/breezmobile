...
class PayerRemoteSession extends RemoteSession with OnlineStatusUpdater {
  ...
  Future _resetSessionState() {
    var sessionResetState =
        PaymentSessionState.payerStart(sessionID, _currentUser.name, _currentUser.avatarURL).copyWith(payeeData: _currentSession.payeeData);
  ...
  Future<void> _sendPaymentDetails(PaymentDetails details) {
    return pushStateUpdate({"amount": details.amount.toInt(), "description": details.description}).then((res) {
      _paymentSessionController.add(_currentSession.copyWith(
          payerData: _currentSession.payerData.copyWith(amount: details.amount.toInt(), description: details.description)));
    });
  }
  ...
  void _watchInviteRequests(SessionLinkModel sessionLink) {
    ...
    _deepLinks
        .generateSessionInviteLink(SessionLinkModel(sessionLink.sessionID, sessionLink.sessionSecret, sessionLink.initiatorPubKey))
        .then((inviteLink) {
      log.info("payer session generating invite link completed");
      _currentSessionInvite = inviteLink;
      _paymentSessionController.add(_currentSession.copyWith(invitationReady: true));
    }).catchError((err) {
    ...
    _sentInvitesController.stream.listen((inviteLink) async {
      ...
      if (shared == true) {
        _paymentSessionController.add(_currentSession.copyWith(invitationSent: true));
      }
    });
  }
