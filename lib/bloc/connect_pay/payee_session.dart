...
class PayeeRemoteSession extends RemoteSession with OnlineStatusUpdater {
  ...
  Future _persistPayerData(PayerSessionData newPayerData) {
    return _breezLib.ratchetSessionSetInfo(
        sessionID,
        json.encode({
          "payerData": {"imageURL": newPayerData.imageURL, "userName": newPayerData.userName}
        }));
  }

  Future _resetSessionState() async {
    var sessionResetState =
        PaymentSessionState.payeeStart(sessionID, _currentUser.name, _currentUser.avatarURL).copyWith(payerData: _currentSession.payerData);
  ...
  void _watchPaymentFulfilled() {
    _invoicesPaidSubscription =
        _breezLib.notificationStream.where((event) => event.type == NotificationEvent_NotificationType.INVOICE_PAID).listen((_) {
