...
class PaymentSessionChannel {
  ...
  PaymentSessionChannel(this._sessionID, this._payer, {this.interceptor}) {
    ...
    Future sessionCreated = !_payer
        ? Future.value(null)
        : FirebaseDatabase.instance.reference().child('remote-payments/$_sessionID').update({"lastUpdated": ServerValue.timestamp});
  ...
  void _watchPeerReset() {
    ...
    _peerResetListener = Observable(terminationRef.onValue).delay(Duration(milliseconds: 500)).listen((event) {
