...
class PayeeSessionData {
  ...
  PayeeSessionData copyWith({String userName, String imageURL, PeerStatus status, String paymentRequest, String error, bool cancelled}) {
    return new PayeeSessionData(userName ?? this.userName, imageURL ?? this.imageURL, status ?? this.status,
        paymentRequest ?? this.paymentRequest, error ?? this.error, cancelled ?? this.cancelled);
  }
}
...
class PayerSessionData {
  ...
  PayerSessionData copyWith(
      {String userName, String imageURL, PeerStatus status, int amount, String description, String error, bool paymentFulfilled}) {
    return new PayerSessionData(
        userName ?? this.userName, imageURL ?? this.imageURL, status ?? this.status, amount ?? this.amount, description ?? this.description,
        error: error ?? this.error, paymentFulfilled: paymentFulfilled ?? this.paymentFulfilled, cancelled: cancelled ?? this.cancelled);
  }
}
...
class PaymentSessionState {
  ...
  PaymentSessionState(this.payer, this.sessionSecret, this.payerData, this.payeeData, this.invitationReady, this.invitationSent,
      this.paymentFulfilled, this.settledAmount);

  PaymentSessionState.payeeStart(String sessionSecret, String userName, String imageURL)
      : this(false, sessionSecret, PayerSessionData(null, null, PeerStatus.start(), null, null),
            PayeeSessionData(userName, imageURL, PeerStatus.start(), null, null, false), true, true, false, 0);

  PaymentSessionState.payerStart(String sessionSecret, String userName, String imageURL)
      : this(true, sessionSecret, PayerSessionData(userName, imageURL, PeerStatus.start(), null, null),
            PayeeSessionData(null, null, PeerStatus.start(), null, null, false), false, false, false, 0);
  ...
  PaymentSessionState copyWith(
      ...
      int settledAmount}) {
    return new PaymentSessionState(
        this.payer,
        this.sessionSecret,
