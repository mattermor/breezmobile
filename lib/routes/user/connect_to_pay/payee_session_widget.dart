...
class _PayeeInstructions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (_sessionState.paymentFulfilled) {
      ...
      return LoadingAnimatedText('Waiting for ${_sessionState.payerData.userName ?? "payer"} to enter an amount',
      ...
      if (_account.maxAllowedToReceive < Int64(_sessionState.payerData.amount)) {
        return Column(
          children: <Widget>[
            ...
            Text('This payment exceeds your limit (${_account.currency.format(_account.maxAllowedToReceive)}).',
                style: theme.sessionNotificationWarningStyle, textAlign: TextAlign.center)
          ],
        );
      }
