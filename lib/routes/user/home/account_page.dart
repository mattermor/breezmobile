...
class AccountPageState extends State<AccountPage> with SingleTickerProviderStateMixin {
  ...
  Widget _buildBalanceAndPayments(PaymentsModel paymentsModel, AccountModel account, String pendingCTPLink) {
    ...
    double bottomPlaceholderSpace = paymentsModel.paymentsList == null || paymentsModel.paymentsList.length == 0
        ? 0.0
        : (listHeightSpace - PAYMENT_LIST_ITEM_HEIGHT * paymentsModel.paymentsList.length).clamp(0.0, listHeightSpace);

    String message = account?.statusMessage;
    bool showMessage =
        account?.syncUIState != SyncUIState.BLOCKING && (account != null && !account.initial || account?.isInitialBootstrap == true);
    ...
    if (paymentsModel.nonFilteredItems.length > 0) {
      ...
      slivers.add(SliverPersistentHeader(pinned: true, delegate: new FixedSliverDelegate(bottomPlaceholderSpace, child: Container())));
    } else if (showMessage) {
      slivers.add(SliverPersistentHeader(
        delegate: new FixedSliverDelegate(200.0, builder: (context, shrinkedHeight, overlapContent) {
          return Container(
            child: Padding(
              padding: const EdgeInsets.only(top: 130.0, left: 40.0, right: 40.0),
              child: (StatusText(message, loading: account?.swapFundsStatus?.error?.isNotEmpty != true)),
            ),
          );
        }),
      ));
    ...
    return Row(
      ...
      children: <Widget>[
        Padding(
          ...
          child: Chip(
            label: new Text(DateUtils.formatFilterDateRange(filter.startDate, filter.endDate)),
            onDeleted: () => _accountBloc.paymentFilterSink.add(PaymentFilterModel(filter.paymentType, null, null)),
          ),
        )
      ],
    );
