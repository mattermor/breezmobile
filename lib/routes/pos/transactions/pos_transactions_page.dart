...
class PosTransactionsPageState extends State<PosTransactionsPage> {
  ...
  Widget _buildTransactions(PaymentsModel paymentsModel) {
    return Stack(
      fit: StackFit.expand,
      children: [
        ((paymentsModel.filter.startDate != null && paymentsModel.filter.endDate != null) && paymentsModel.paymentsList.length == 0)
  ...
  Widget _calendarButton(PaymentsModel paymentsModel) {
    return Padding(
        ...
        child: IconButton(
            ...
            onPressed: () => showDialog(context: context, builder: (_) => CalendarDialog(context, paymentsModel.firstDate)).then(
                ((result) =>
                    _accountBloc.paymentFilterSink.add(paymentsModel.filter.copyWith(startDate: result[0], endDate: result[1]))))));
  }
